...
namespace SOLUTION_OF_THE_DIRICHLET_PROBLEM_FOR_THE_POISSON_EQUATION
{
    internal class MethodSimpleIter
    {
        ...
        public void MSI(ref List<List<double>> matrix, ref List<double> x_temp, ref List<double> y_temp, ref int S, ref double eps_max)
        {
            ...
            while (!check && S < N_Max)
            {
                ...
                for (int j = 1; j < M; ++j)
                {
                    ...
                    {
                        v_old = matrix[i][j];
                        v_new = v_old + tau * residual_vec[(i - 1) + (j - 1) * (N - 1)];
        ...
        private void residual_vec_calc(ref List<List<double>> matrix)
        {
            ...
            for (int j = 1; j < M; ++j)
            {
                for (int i = 1; i < N; ++i)
                {
                    ...
                    residual_vec.Add(left_part_of_eq - right_part_v[(i - 1) + (j - 1) * (N - 1)]);
                }
            }
