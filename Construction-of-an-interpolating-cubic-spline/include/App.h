...
private:
    ...
    bool isShowNumericalSolutionSet = false; // показывать ли численное решение
...
public:
    ...
    void createWindowForSettings() {
        ...
        ImGui::Checkbox("Show NumericalSolut", &isShowNumericalSolutionSet);
    ...
    void create_data_for_numerical_solution() {
        ...
        for (size_t count = 0; count < cs.get_n(); ++count) {
            ...
            double h = (double)(abs((cs.get_border().first + count * (abs(cs.get_border().second - cs.get_border().first) / cs.get_n())) -
                (cs.get_border().first + (count + 1) * (abs(cs.get_border().second - cs.get_border().first) / cs.get_n()))) / separate_n); // шаг в интервале для S_i(x)
    ...
    void createGraph() { // функция для отрисовки графика
        ...
        if (ImPlot::BeginPlot("Solution schedule", "x", "y", { 940,780 })) { // Отрисовываем график
            
            if (isAnalyticalSolutionSet && isShowAnalyticalSolution)
                ImPlot::PlotLine("Analytical solution graph", x_of_analytical_solution.data(), y_of_analytical_solution.data(), x_of_analytical_solution.size()); // Отрисовываем линию

            if (isNumericalSolutionSet && isShowNumericalSolutionSet)
                ImPlot::PlotLine("Numerical solution graph", x_of_numerical_solution.data(), y_of_numerical_solution.data(), x_of_numerical_solution.size()); // Отрисовываем линию
